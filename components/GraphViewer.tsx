...
const NODE_COLORS: Record<string, string> = {
    ...
    address: '#facc15', // Yellow
    service: '#22d3ee', // Cyan
    promotion: '#a3e635', // Lime
    agreement: '#f87171', // Red
    shipment: '#fb923c', // Orange Light
    array: '#6b7280',   // Gray
    object: '#a1a1aa',  // Gray Light
    default: '#d4d4d8' // Gray Lighter
};
...
const GraphViewer: React.FC<GraphViewerProps> = ({ data }) => {
    ...
    useEffect(() => {
        ...
        const getNodeStatusColor = (d: NodeType) => {
            if (d.status?.applicableMilestone) {
                return STATUS_BORDER_COLORS[d.status.applicableMilestone.toLowerCase()] || STATUS_BORDER_COLORS.default;
            }
            if (d.status?.statusCode) {
                return STATUS_BORDER_COLORS[d.status.statusCode] || STATUS_BORDER_COLORS.default;
            }
    ...
    return (
        <div className="relative w-full h-full bg-gray-800/50 rounded-lg border border-gray-700 overflow-hidden">
            ...
            {tooltip.visible && (
                <div
                    className="absolute p-2 text-xs bg-gray-900/90 text-gray-200 border border-gray-600 rounded-md shadow-lg pointer-events-none max-w-sm max-h-80 overflow-auto backdrop-blur-sm"
